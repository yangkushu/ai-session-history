...
仓库维护一份符合开放 Agent Skills 格式的核心 Skill，主要通过通用
`npx skills add` CLI 安装到三端，手动复制作为无 Node.js 或离线环境的 fallback。
Skill 安装不得修改任何 agent permission 配置；运行时权限仍由用户和对应 agent
host 授予。
...
- 支持通过 `npx skills add` 安装到 Codex、Claude Code 和 Cursor，并提供手动 fallback。
...
`npx skills` 负责 host 目录映射、全局或项目级安装以及后续 update/remove。Node.js 和
网络仅是 Skill 安装时的依赖，不成为 `ai-history` Go binary 的运行时依赖。用户应在
执行前审阅来源和 Skill 内容；仓库文档不得暗示 `npx` 安装会授予 history 读取权限。

没有 Node.js、没有网络或不愿使用第三方 installer 时，文档提供把
`skills/ai-history` 复制到 host 官方 Skill 目录的手动 fallback。实现前和发布前均需
复核通用 CLI 的 agent identifiers 以及三端官方目录。无论采用哪种方式，都只安装
Skill 文件，不修改 permission、sandbox、allowlist 或 managed policy 配置。
...
- `npx skills` 能发现 `ai-history`，并识别 Codex、Claude Code 和 Cursor target。
- 使用隔离的临时目录验证安装，不污染真实用户目录。
- 手动 fallback 使用同一份核心 Skill，不维护平台分叉副本。
- 安装文档明确说明安装 Skill 不会自动授予 CLI 或 history 权限。
...
- 本机缺少的平台通过 `npx skills` 的 target discovery 和文档审查验证，并记录未执行的
  人工验收。
