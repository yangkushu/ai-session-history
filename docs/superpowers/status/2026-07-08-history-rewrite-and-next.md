...
OpenSpec change `bootstrap-ai-session-history-cli` remains open. Cursor latest
macOS validation has now been completed from a real local sample:

- observed macOS `globalStorage/state.vscdb` with `cursorDiskKV`
  `composerData:<composerId>` keys;
- captured a minimized empty-composer SQL fixture with private content removed;
- added synthetic non-private macOS composerData tests for `list` and `show`;
- implemented macOS composerData parsing in `internal/readers/cursor.go`.

The remaining OpenSpec completion work is to archive the change after final
verification and decide whether to keep any follow-up P1 notes.
